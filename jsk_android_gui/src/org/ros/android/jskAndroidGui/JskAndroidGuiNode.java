...
public class JskAndroidGuiNode extends AbstractNodeMain {
	...
	private ArrayList<String> spots_list,tasks_list, image_list,camera_info_list, points_list;
	...
	public void getTasksParam(){
		...
		NameResolver resolver_user = connectedNode.getResolver().newChild( guser);
		Object[] user_list = parameterTree.getList(resolver_user.resolve("UserList")).toArray();
		tasks_list.clear();
		tasks_list.add("tasks");
		for (int i = 0; i < user_list.length; i++) {
			GraphName gtask = GraphName.of(defaultTask_ns + "/User");
			NameResolver resolver_task = connectedNode.getResolver().newChild(gtask);
			Object[] task_param_list = parameterTree.getList(resolver_task.resolve((String)user_list[i])).toArray();
	...
	public void getCameraParam(){
		...
		NameResolver resolver_camera = connectedNode.getResolver().newChild(gcamera);
		Object[] camera_names_list = parameterTree.getList(resolver_camera.resolve("CameraList")).toArray();
		...
		for (int i = 0; i <camera_names_list.length; i++) {
			Object[] camera_param_list = parameterTree.getList(resolver_camera.resolve((String)camera_names_list[i])).toArray();
//			if (i == 0) {
...
//			}
	...
	public void  getPointsParam(){
		...
		NameResolver resolver_point = connectedNode.getResolver().newChild(gparam);
		Object[] points_param_list = parameterTree.getList(resolver_point.resolve("points")).toArray();
