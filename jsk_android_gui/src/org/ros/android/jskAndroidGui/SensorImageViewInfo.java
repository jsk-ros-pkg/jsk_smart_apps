...
public class SensorImageViewInfo extends ImageView implements
	...
	private boolean isDrawLine = false, isMovingFingerInfo = false,
			isPushOnce = false, isPickOnce = false, isPlaceOnce = false,
	...
	private ArrayList<Integer> startXList = new ArrayList(),
			startYList = new ArrayList(), curXList = new ArrayList(),
			curYList = new ArrayList(), fingerList = new ArrayList();
	...
	public void SendCommandMsg(String task_name, int arm_id, String state,
		...
		for (int i = 0; i <= xList.size() - 1; i++) {
			...
			TouchMsg.setX(xList.get(i));
			TouchMsg.setY(yList.get(i));
			TabletMsg.getTouches().add(TouchMsg);
		}
		if (touch_x != 0)
			TabletMsg.getAction().setTouchX(touch_x);
		if (touch_y != 0)
			TabletMsg.getAction().setTouchY(touch_y);
	...
	public void ZoomCamera(int touch_x, int touch_y, float movement) {
		SendCommandMsg("ZoomCamera", 0, "PINCH", movement, null, 0, fingerList,
				startXList, startYList, touch_x, touch_y);
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		Log.i("JskAndroidGui:TouchEvent", "current fingerCount = "
				+ fingerCount + ", current fingerCount_temp = "
				+ fingerCount_temp);
		...
		case MotionEvent.ACTION_POINTER_DOWN: {
			...
			Log.i("JskAndroidGui:TouchEvent",
					"[ACTION_POINTER_DOWN] current fingerCount = "
							+ fingerCount);

			startXList
					.add((int) (event.getX(fingerCount - 1) * DefaultWidth / MaxWidth));
			startYList
					.add((int) (event.getY(fingerCount - 1) * DefaultWidth / MaxWidth));
		...
		case MotionEvent.ACTION_MOVE: {
			...
			Log.i("JskAndroidGui:TouchEvent", "[ACTION_MOVE] curXList = "
					+ curXList + ", curYList = " + curYList);
			SendDebugMsg("ActionMove", fingerCount, curXList.get(0),
					curYList.get(0));
			if (isDrawLine && fingerCount == 2 && fingerCount_temp == 2) {
				SendTaskMsg("DrawLine", 0, "TOUCHMOVE", 0, null, 0, fingerList,
						curXList, curYList, 0, 0);
		...
		case MotionEvent.ACTION_UP: {
			...
			if (fingerCount == 1 && curXList.size() == 0) {
				...
				if (isPushOnce) {
					...
					SendCommandMsg("PushOnce", 0, "TOUCH", 0, null, 0,
							...
							startXList.get(0), startYList.get(0));
				} else if (isPickOnce) {
					isPickOnce = false;
					// SendCommandMsg("PickOnce", 0, "TOUCH", 0, null, 0,
					SendCommandMsg("PickObjectSelected", RobotArmId, "TOUCH",
							0, null, 0, fingerList, startXList, startYList,
							startXList.get(0), startYList.get(0));
				} else if (isPlaceOnce) {
					isPlaceOnce = false;
					...
					isPassToHumanOnce = false;
					SendCommandMsg("PassToHumanOnce", RobotArmId, "TOUCH", 0,
							null, 0, fingerList, startXList, startYList,
							startXList.get(0), startYList.get(0));
				} else if (TouchMode == 0) {
				...
				final float swipeLength = Math.round(Math.sqrt(Math.pow(
						curXList.get(curXList.size() - 1) - startXList.get(0),
						2)
						+ Math.pow(curYList.get(curYList.size() - 1)
								- startYList.get(0), 2)));//

				if (swipeLength >= minLength) {
					SendDebugMsg("SWIPE", swipeLength, curXList.get(0),
							curYList.get(0));
					final int X = startXList.get(0)
							- curXList.get(curXList.size() - 1);
					final int Y = curYList.get(curYList.size() - 1)
							- startYList.get(0);
			...
			} else if (fingerCount == 2 && curXList.size() > 0
					&& fingerCountOver == 0) {
				/*
				 * if fingercount == 2, startlist.size() = 2, curlist.size =
				 * 1or2
				 */
				...
				final float endDistance = Math.round(Math.sqrt(Math.pow(
						curXList.get(curXList.size() - 1) - curXList.get(0), 2)
						+ Math.pow(
								curYList.get(curYList.size() - 1)
										- curYList.get(0), 2)));
				final float pinchLength = startDistance - endDistance;

				final int touch_x = (curXList.get(0) + curXList.get(curXList
						.size() - 1)) / 2;
				final int touch_y = (curYList.get(0) + curYList.get(curYList
						.size() - 1)) / 2;
				SendDebugMsg("PINCH", pinchLength, touch_x, touch_y);
				if (false) {// get camera id?
					ZoomCamera((startXList.get(0) + startXList.get(1)) / 2,
							(startYList.get(0) + startYList.get(1)) / 2,
							pinchLength);
				} else {
				...
				if (startYList.get(0) >= DefaultHeight / 2
					...
					&& (Math.abs(startXList.get(0) - curXList.get(0)) > minLength)) {
		...
		case MotionEvent.ACTION_POINTER_UP: {
			...
			Log.v("JskAndroidGui:TouchEvent",
					"[ACTION_POINTER_UP] fingerCount = " + fingerCount
							+ ", fingerCount_temp = " + fingerCount_temp);
			Log.i("JskAndroidGui:TouchEvent",
			...
			if (fingerCount_temp == 2 && fingerCount == 2 && isMovingFingerInfo) {
				...
				Log.v("JskAndroidGui:TouchEvent",
						"[ACTION_POINTER_UP] startXList = " + startXList
								+ ", startYList = " + startYList);
				Log.v("JskAndroidGui:TouchEvent",
						"[ACTION_POINTER_UP] curXList = " + curXList
								+ ", curYList = " + curYList);
	...
	public GraphName getDefaultNodeName() {
		return GraphName.of("jsk_android_gui/SendorImageViewInfo");
	}
}
