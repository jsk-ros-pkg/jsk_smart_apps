...
public class JskAndroidGui extends RosAppActivity {
	...
	protected void onNodeCreate(Node node) {
		...
		yes_button.setOnClickListener(new OnClickListener() {
			public void onClick(View viw) {
				...
				StrMsg_resultyes.header.stamp = Time.fromMillis(System
						.currentTimeMillis());
		...
		no_button.setOnClickListener(new OnClickListener() {
			public void onClick(View viw) {
				...
				StrMsg_resultno.header.stamp = Time.fromMillis(System
						.currentTimeMillis());
		...
		try {
			...
			NameResolver resolver_spot = node.getResolver().createResolver(
					gspot);
			Object[] spots_param_list = params.getList(
			...
			Toast.makeText(JskAndroidGui.this,
					"No Param Found: " + ex.getMessage(), Toast.LENGTH_SHORT)
					.show();
		}
		spots_spinner.setOnItemSelectedListener(new OnItemSelectedListener() {
			public void onItemSelected(AdapterView parent, View viw, int arg2,
				...
				if (isAdapterSet_spots) {
					...
					StrMsg.header.stamp = Time.fromMillis(System
							.currentTimeMillis());
					StrMsg.data = item;
		...
		try {
			...
			NameResolver resolver_user = node.getResolver().createResolver(
					guser);
			Object[] user_list = params.getList(
			...
			for (int i = 0; i < user_list.length; i++) {
				...
				NameResolver resolver_task = node.getResolver().createResolver(
						gtask);
				Object[] task_param_list = params.getList(
				...
				for (int j = 0; j < task_param_list.length; j++) {
					Log.i("JskAndroidGui:GetTasksParam", "lists: " + i + " "
							+ j + " /Tablet/" + (String) user_list[i] + "/"
		...
		try {
			...
			NameResolver resolver_camera = node.getResolver().createResolver(
					gcamera);
		...
		try {
			...
			NameResolver resolver_point = node.getResolver().createResolver(
					gparam);
			Object[] points_param_list = params.getList(
				...
				new ParameterListener() {
					...
					public void onNewValue(Object value) {
						try {
							String defaultTask_ns = "/Tablet";
							GraphName guser = new GraphName(defaultTask_ns);
							NameResolver resolver_user = public_node
									.getResolver().createResolver(guser);
							Object[] user_list = params.getList(
									resolver_user.resolve("UserList"))
									.toArray();
							tasks_list.clear();
							tasks_list.add("tasks");
							for (int i = 0; i < user_list.length; i++) {
								GraphName gtask = new GraphName(defaultTask_ns
										+ "/User");
								NameResolver resolver_task = public_node
										.getResolver().createResolver(gtask);
								Object[] task_param_list = params
										.getList(
												resolver_task
														.resolve((String) user_list[i]))
										.toArray();
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.stopjoint:
			...
			StrMsg_stopjoint.header.stamp = Time.fromMillis(System
					.currentTimeMillis());
		...
		case R.id.stopnavigation:
			...
			StrMsg_stopnavigation.header.stamp = Time.fromMillis(System
					.currentTimeMillis());
		...
		case R.id.switchjoy:
			...
			StrMsg_switchjoy.header.stamp = Time.fromMillis(System
					.currentTimeMillis());
